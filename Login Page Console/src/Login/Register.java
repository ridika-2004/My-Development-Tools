...
public class Register {
    ...
    public void register(String fileString){
        while (true) {
            ...
            if(!Validator.validator(username,fileString,0) && !Validator.validator(email,fileString,1)){
                saveDetails(username+","+email+","+HashPassword.hashPassword(password), fileString);
                break;
            }
            else{
                System.out.println("This username or email already exists. Please try a another one");
            }
        }
    }
