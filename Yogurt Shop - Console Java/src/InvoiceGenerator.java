...
public class InvoiceGenerator {
    ...
    private void writeFlavorsToInvoice(Order order, FileWriter writer) throws IOException {
        List<YogurtFlavor> processedFlavors = new ArrayList<>();
        for (YogurtFlavor flavor : order.getFlavors()) {
            if (!processedFlavors.contains(flavor)) {
                int count = 0;
                for (YogurtFlavor f : order.getFlavors()) {
                    if (f.equals(flavor)) {
                        count++;
                    }
                }
                writer.write(flavor.getName() + " - " + count + " scoop: $" + String.format("%.2f", flavor.getPricePerScoop() * count) + "\n");
    ...
    private void writeToppingsToInvoice(Order order, FileWriter writer) throws IOException {
        ...
        for (Topping topping : order.getToppings()) {
            if (!processedToppings.contains(topping)) {
                ...
                writer.write(topping.getName() + " - " + count + " time: $" + String.format("%.2f", topping.getPrice() * count) + "\n");
